...
void main() {
    vec4 graphBase = vec4(1, 1, 1, 1);
    vec4 sampled = uUseDiffuseTexture ? texture(uDiffuseTexture, vUV) : vec4(1.0);
    vec3 normalTint = normalize(vWorldNormal) * 0.5 + 0.5;
    vec3 emissive = vec3(0, 0, 0);
    float metallic = clamp(0, 0.0, 1.0);
    float roughness = clamp(0.5, 0.02, 1.0);
    vec4 base = graphBase * sampled * uColor;
