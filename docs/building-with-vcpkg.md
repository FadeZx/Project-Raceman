...
Project Raceman depends on GLFW, GLAD, and Assimp. The Visual Studio project now
uses vcpkg manifest mode to download and integrate these libraries
automatically. The steps below assume that you already have the vcpkg tool
...
5. Build the solution as usual (Debug or Release). The manifest makes sure that
   the linker finds the GLFW, GLAD, and Assimp symbols that previously caused
