        ...
        try {
            ...
            if (root.is_object()) {
                ...
                if (graphicsIt != object.end() && graphicsIt->second.is_object()) {
                    ...
                    if (auto it = graphics.find("weatherDrawDistance"); it != graphics.end() && it->second.is_number()) graphicsProfile_.weatherDrawDistance = (std::clamp)(static_cast<float>(it->second.as_number()), 15.0f, 120.0f);
                    if (auto it = graphics.find("weatherStreakLength"); it != graphics.end() && it->second.is_number()) graphicsProfile_.weatherStreakLength = (std::clamp)(static_cast<float>(it->second.as_number()), 0.1f, 2.0f);
                    if (auto it = graphics.find("weatherStreakWidth"); it != graphics.end() && it->second.is_number()) graphicsProfile_.weatherStreakWidth = (std::clamp)(static_cast<float>(it->second.as_number()), 0.002f, 0.08f);
                    if (auto it = graphics.find("weatherFallSpeedScale"); it != graphics.end() && it->second.is_number()) graphicsProfile_.weatherFallSpeedScale = (std::clamp)(static_cast<float>(it->second.as_number()), 0.2f, 3.0f);
...
void SceneEditor::SaveProject() {
    try {
        ...
        out << "    \"weatherDrawDistance\": " << graphicsProfile_.weatherDrawDistance << ",\n";
        out << "    \"weatherStreakLength\": " << graphicsProfile_.weatherStreakLength << ",\n";
        out << "    \"weatherStreakWidth\": " << graphicsProfile_.weatherStreakWidth << ",\n";
        out << "    \"weatherFallSpeedScale\": " << graphicsProfile_.weatherFallSpeedScale << ",\n";
